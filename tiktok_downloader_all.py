...
def download_tiktok_video(url, index, total, config):
    ...
    try:
        ...
        if not downloader:
            print(f"[{index}/{total}] ⚠️ Keine Download-Links gefunden für {url}")
            return
        ...
        if config['skip_existing'] and os.path.exists(path):
            print(f"[{index}/{total}] ⏭️ Datei {fname} existiert bereits")
            return

        video.download(path)
        print(f"[{index}/{total}] ✅ Video gespeichert: {fname}")
    except Exception as e:
        print(f"[{index}/{total}] ❌ Fehler bei Video: {e}")
...
def download_tiktok_photo(url, index, total, config):
    ...
    try:
        downloader = snaptik(url)
        if not downloader:
            print(f"[{index}/{total}] ⚠️ Keine Bilder gefunden für {url}")
            return
        ...
        for i, item in enumerate(downloader, start=1):
            ...
            item.download(path)
            print(f"[{index}/{total}] ✅ Bild gespeichert: {fname}")
    except Exception as e:
        print(f"[{index}/{total}] ❌ Fehler bei Bild: {e}")
...
def process_url_file():
    ...
    if not os.path.exists(urls_file):
        print("⚠️ urls.txt nicht gefunden.")
        return
    ...
    with ThreadPoolExecutor(max_workers=config['threads']) as executor:
        ...
        for idx, url in enumerate(urls):
            if '/video/' in url:
                ...
                print(f"[{idx+1}/{total}] ⚠️ Unbekannter TikTok-Link: {url}")
    ...
    print("\n✅ Alle Inhalte wurden verarbeitet.")
